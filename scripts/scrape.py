...
def taipeitheater_login(session: requests.Session):
    ...
    resp = session.post(
        f"{TAIPEI_BASE}/login.php",
        data={"account": user, "password": pwd},
        timeout=20,
    )
    resp.raise_for_status()
    # 驗證登入成功：頁面應該出現「登出」字樣
    check = session.get(f"{TAIPEI_BASE}/tbopercentage2.php", timeout=20)
...
def internal_login(session: requests.Session):
    ...
    login_page = session.get(f"{INTERNAL_BASE}/Account/Login", timeout=20)
    ...
    resp = session.post(f"{INTERNAL_BASE}/Account/Login", data=data, timeout=20)
    resp.raise_for_status()
    check = session.get(f"{INTERNAL_BASE}/Report/Index", timeout=20)
...
def fetch_internal_range(session: requests.Session, d1: str, d2: str):
    ...
    resp = session.get(
        f"{INTERNAL_BASE}/Report/HistoryReport",
